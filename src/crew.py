...
class SEOAnalyseCrew():
    ...
    def test_pdf_conversion(self):
        """Test function to convert an existing report.md to PDF without running the crew"""
        try:
            # Read the existing report
            with open('report.md', 'r', encoding='utf-8') as f:
                markdown_content = f.read()
            
            # Set a test website URL if none exists
            if not hasattr(self, 'website_url'):
                self.website_url = "test.com"
            
            # Call the convert_to_pdf method
            self.convert_to_pdf()
            print("Test PDF conversion completed successfully")
        except Exception as e:
